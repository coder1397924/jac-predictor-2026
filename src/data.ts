...
export const ACTUAL_R3_2026: Record<string, Record<string, number>> = {
  "GNGND": {
    "DTU_COE": 12966,
    "DTU_CSDS": 13672,
    "DTU_IT": 16920,
...
export function getPredictionsForCategory(category: CategoryCode): Record<BranchCode, Prediction2026> {
  ...
  for (const branch of branchList) {
    ...
    const r3 = ACTUAL_R3_2026[category]?.[branch] ?? 0;
    if (r1 === 0 || (branch.startsWith('IGDTUW') && !isGL)) {
      predictions[branch] = { actualR1: 0, actualR2: 0, actualR3: 0, worstCase: { r1: 0, r2: 0, r3: 0, r4: 0, r5: 0, upgradation: 0 }, trueOutcome: { r1: 0, r2: 0, r3: 0, r4: 0, r5: 0, upgradation: 0 } };
      continue;
    }
    const jumpPercent = computeJumpPercent(category, r1, isOS);
    const totalJump = computeTotalJump(r1, jumpPercent, isOS);

    const baseR = r3 || r2 || r1;

    const worstCase = { 
      r1, 
      r2: r2 || Math.round(r1 + totalJump * 0.25), 
      r3: r3 || Math.round((r2 || r1) + totalJump * (r2 ? 0.15 : 0.35)), 
      r4: Math.round(baseR + totalJump * (r3 ? 0.10 : r2 ? 0.25 : 0.45)), 
      r5: Math.round(baseR + totalJump * (r3 ? 0.20 : r2 ? 0.45 : 0.65)), 
      upgradation: Math.round(baseR + totalJump * (r3 ? 0.35 : r2 ? 0.55 : 0.75)) 
    };

    const trueOutcome = { 
      r1, 
      r2: r2 || Math.round(r1 + totalJump * 0.35), 
      r3: r3 || Math.round((r2 || r1) + totalJump * (r2 ? 0.25 : 0.50)), 
      r4: Math.round(baseR + totalJump * (r3 ? 0.20 : r2 ? 0.45 : 0.65)), 
      r5: Math.round(baseR + totalJump * (r3 ? 0.40 : r2 ? 0.65 : 0.85)), 
      upgradation: Math.round(baseR + totalJump * (r3 ? 0.65 : r2 ? 0.85 : 1.0)) 
    };

    predictions[branch] = { actualR1: r1, actualR2: r2, actualR3: r3, worstCase, trueOutcome };
  }
